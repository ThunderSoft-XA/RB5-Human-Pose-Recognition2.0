...
import com.thundercomm.eBox.AI.HumanPoseRecognitionTask;
...
public class MyGLSurfaceView extends GLSurfaceView {
    ...
    private void getAlgorithmType() {
        if (getAlgorithmType(surfaceId, Constants.ENABLE_HUMANPOSERECOGNITION_STR)) {
            p = new PutIntoHumanPoseRecognitionTask();
        } else {
            p = null;
        }
    }
    ...
    private void getObjectInfo(int surfaceId, Mat mat) {
        //todo: 上传
        HumanPoseRecognitionTask.getHumanPoseRecognitionTask().addMatById(surfaceId, mat, mOutputWidth, mOutputHeight);
    }
    ...
    private class PutIntoHumanPoseRecognitionTask implements PutIntoTask {
        private long pts = 0;

        @Override
        public void putInTask(int surfaceId, Mat matFromGlSurface, GL10 arg0) {
            if (HumanPoseRecognitionTask.getHumanPoseRecognitionTask().isIstarting()) {
                if (System.currentTimeMillis() - pts >= Constants.INPUT_HUMANPOSERECOGNITION_INTERVAL) {
